...
from POST.postprocess.utils import ObjectPredictionList, has_match, merge_object_prediction_pair
from POST.models.prediction import ObjectPrediction
from POST.utils.import_utils import check_requirements
...
def diou_nms(boxes, scores, iou_threshold=0.5):
    """
    Perform DIoU-based Non-Maximum Suppression (NMS).

    Args:
        boxes (Tensor): Bounding boxes, shape (N, 4), format [x1, y1, x2, y2].
    ...
    while order.numel() > 0:
        ...
        center_x2, center_y2 = (x1[order] + x2[order]) / 2, (y1[order] + y2[order]) / 2
        center_dist = (center_x1[i] - center_x2) ** 2 + (center_y1[i] - center_y2) ** 2

        enclose_x1 = torch.min(x1[i], x1[order])
        enclose_y1 = torch.min(y1[i], y1[order])
        enclose_x2 = torch.max(x2[i], x2[order])
        enclose_y2 = torch.max(y2[i], y2[order])
        enclose_diag = (enclose_x2 - enclose_x1) ** 2 + (enclose_y2 - enclose_y1) ** 2

        diou = iou - center_dist / enclose_diag.clamp(min=1e-6)

        # Keep boxes with DIoU below the threshold
        order = order[diou <= iou_threshold]

    return keep


class DIOUNMSPostprocess(PostprocessPredictions):
    def __call__(
        self,
        object_predictions: List[ObjectPrediction],
    ):
        ...
        keep = diou_nms(
            boxes, scores, iou_threshold=self.match_threshold
        )

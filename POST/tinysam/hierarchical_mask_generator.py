...
class SamHierarchicalMaskGenerator:
    ...
    def generate_from_prompt(self,image: np.ndarray, boxes, need_high: bool):
        new_masks = self.generate_from_auto_prompt(image,boxes, need_high)
        #new_masks.cat(ori_masks)
        new_masks = self.post_process(image, new_masks)
        return new_masks
    ...
    @torch.no_grad()
    def generate_from_auto_prompt(self, image: np.ndarray,boxes, need_high: bool) -> MaskData:
        orig_size = image.shape[:2]

        # Generate masks for this crop in batches
        data = MaskData()
        for (points,) in batch_iterator(self.points_per_batch, boxes):
            orig_h, orig_w = orig_size
            # Run model on this batch
            #transformed_points = self.predictor.transform.apply_coords(points, orig_size)
